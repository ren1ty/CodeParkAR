    ...
    public class PlaceAtLocation : MonoBehaviour
    {
        ...
        public Location Location
        {
            ...
            set
            {
                ...
                UpdatePosition(locationProvider.CurrentLocation.ToLocation(), true, true);
            }
        }
        ...
        public double NormalDist;
        public bool hasDo = false;
        private void SetLoc()
        {
            if (!hasDo) return;
            int indexOfBestLocation = 0;
            double minDist = 999999;
            for (int i = 0; i < loc.Count; i++)
            {
                double temp = Math.Abs(KnowDist(loc[i]) - NormalDist);
                if (temp < minDist)
                {
                    minDist = temp;
            ...
            transform.position = Location.GetGameObjectPositionForLocation(
                      arLocationRoot, mainCameraTransform, loc[indexOfBestLocation], state.Location, false
                  );
            cant = true;

            var t = transform.position;
            transform.position = new Vector3(t.x, t.y - 1.38f, t.z);

        }
        private double KnowDist(Location objLoc)
        {
            return DistanceBetweenPlaces(Input.location.lastData.longitude, Input.location.lastData.latitude, objLoc.Longitude, objLoc.Latitude);
        }
        ...
        public void UpdatePosition(Location deviceLocation, bool forceUpdate = false, bool isOne = false)
        {
